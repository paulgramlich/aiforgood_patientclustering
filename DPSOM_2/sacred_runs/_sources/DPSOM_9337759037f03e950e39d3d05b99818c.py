...
def evaluate_model(model, generator, len_data_val, x, modelpath, epochs, batch_size, latent_dim, som_dim,
    ...
    return results'''
        ...
        for i in range(num_batches):
            ...
            test_k = sess.run(model.k, feed_dict={x: batch_data, is_training: True, z: np.zeros((batch_size, latent_dim))})
            test_k_all.extend(test_k)

        test_k_all = np.array(test_k_all)
        labels_val_all = np.array(labels_val_all)
        data_val_all = np.array(data_val_all)

        test_nmi = normalized_mutual_info_score(labels_val_all, test_k_all, average_method='geometric')
        test_purity = cluster_purity(test_k_all, labels_val_all)
        test_ami = adjusted_mutual_info_score(labels_val_all, test_k_all)

        # Calculate additional clustering evaluation metrics
        silhouette_avg = silhouette_score(data_val_all, test_k_all)
        calinski_harabasz = calinski_harabasz_score(data_val_all, test_k_all)
        davies_bouldin = davies_bouldin_score(data_val_all, test_k_all)
    ...
    f.write(
        ...
        % (
        results["NMI"], results["AMI"], results["Purity"], results["Silhouette Score"], results["Calinski-Harabasz Index"],
        results["Davies-Bouldin Index"], ex_name))
    f.close()
    return results
...
def main(latent_dim, som_dim, learning_rate, decay_factor, alpha, beta, gamma, theta, ex_name, more_runs, data_set,
    ...
    if data_set == "MNIST":
        ...
        print(f"Min: {np.min(data_train)}")
        print(f"Max: {np.max(data_train)}")
